...
> **Thanh toán:** Dùng QR tĩnh — ảnh cố định từ `STATIC_QR_URL` trong env (QR chuyển khoản ngân hàng). Khách hàng chuyển khoản với nội dung `DH{order_id}` và đúng số tiền. Webhook `POST /webhooks/payment` mô phỏng xác nhận tự động và cập nhật đơn hàng — không cần admin xác nhận thủ công.

| Method | Endpoint | Auth | Mô tả | Logic end-to-end |
|--------|----------|------|-------|------------------|
| POST | `/orders` | Public | Tạo đơn hàng | Validate thông tin giao hàng → lấy giỏ từ Redis → validate coupon nếu có → tính subtotal, discount, shipping_fee, total → insert `orders` (`payment_status='pending'`, `order_status='pending'`) + `order_items` + `order_item_options` (snapshot) → xóa giỏ Redis → lấy `STATIC_QR_URL` từ env (QR ảnh tĩnh ngân hàng) → kiểm tra email/phone chưa có tài khoản active thì tạo user `is_active=false` → gửi email/SMS xác nhận đơn + kích hoạt tài khoản (kèm QR cố định và nội dung chuyển khoản: số tiền + mã đơn `DH{order_id}`) → trả về `order_id`, tóm tắt đơn, `payment_qr_url`, `transfer_content` |
...
| PATCH | `/orders/me/:id/cancel` | Member | Hủy đơn | Kiểm tra thuộc user → kiểm tra status còn `pending` → update `order_status='cancelled'` → hoàn `loyalty_points` nếu đã dùng → insert `loyalty_logs` âm → trả về |
